...
- **一个任务一棵树（git worktree）** —— 会话可以跑在自己的链接工作树、自己的分支上，桌面端侧栏一键创建（宿主侧就是 `cluxmate worktree …`），并行会话不再互相抢同一批文件。树内输入框不再提供分支切换（这棵树本身就是那条分支），项目树的分支列表也会隐藏其他树占用的分支；删除会话时可以选择连树一起移除。参见 [会话级 worktree](#会话级-worktree)。
...
未信任（含"不信任"与"还没回答"）时，以上项目级配置**一律不读**，`permissions.json` 也不写：写不进磁盘的规则等于没有规则。所以审批卡不提供裸的"总是允许"：对**本可记住**的调用，它给的是「**总是允许并信任此项目**」（一次点击同时信任该目录并落盘规则）；而对你明确拒绝过的目录，它只提供「信任此目录」这一步——已记录的拒绝不会被"顺手授权工具"的同一次点击抹掉。全局 `~/.cluxmate/` 配置与内置默认不受影响。未信任不等于只读：文件写入、沙箱边界与审批档位照旧。信任决定记录在 `~/.cluxmate/trust.json`（按目录，精确匹配；不在可写范围内，模型改不了）。

- CLI：`cluxmate trust status` 看当前目录的判定与被拦下的项，`cluxmate trust add` 预授权（headless/CI 用）。
- 桌面：首次接触时弹卡片；工作目录栏有一个可点击的「未信任」徽标（这是不带项目配置的目录唯一会得到的信号）；设置 → 项目信任 可查看与撤销。
...
新树落在 `<repo>/.worktrees/<slug>`，分支 `cluxmate/<slug>`（slug 来自 `--name`，否则 `--title`，再否则 `wt`；重名自动加 `-2`、`-3`…），并把 `.worktrees/` 写进 `<git-common-dir>/info/exclude`，主树不再把它当未跟踪文件——而这个忽略是**前置条件，不是副作用**：`create` 会在建树**之前**写好它，若 git 仍不忽略该容器就以 `exclude-failed` 失败，因为没被忽略的容器会被 `git add -A` 记成一个个内嵌仓库。`create` 默认拒绝有未提交改动的主树（`--allow-dirty` 放行）；`remove` 默认拒绝脏工作树（`--force` 丢弃），`--keep-branch` 保留分支。树内的 `<worktree>/.cluxmate/` 只放运行态——trust、权限、技能、MCP、hooks 与 `AGENTS.md` 仍然读写**主仓**的那一份。

### 桌面端怎么用

- **创建** —— 项目（项目头部、项目菜单或分组头部）上的「在 git 工作树中新建 Session」打开一个对话框：名称（目录与分支都由它派生）、基准分支、可选的分支名。主树有未提交改动会阻止创建，对话框就在原地提供「提交改动」（`chore: WIP`）或「暂存改动」。只有**确认不是 git 仓库**的项目才隐藏该入口。
- **知道自己在哪** —— 会话带一个工作树名称徽标（分支名在 tooltip 里），并归入项目自己的分组。
- **移除** —— 会话右键菜单（以及删除会话本身）会问是否连带移除工作树（树目录 + 分支 + 会话，或仅删会话，并提示此后只能靠 `cluxmate worktree remove` 找到它）。删除整个项目会对其中的工作树逐个问同样的问题，并如实报告没能移除的树，而不是谎报成功。
- **安全护栏** —— 只要还有**别的**会话活在那棵树里就拒绝移除；输入框的分支胶囊在工作树内**禁用**并显示树名（树被钉在创建时的分支上），项目树的分支下拉也会隐藏链接工作树正占用的分支；新建的**普通**会话不会继承工作树目录——它会开在项目里。
- **容器是 CluxMate 的，不是你的改动** —— 桌面端从不把 `.worktrees/` 算作你的未提交改动，「提交改动」那条路径也拒绝把未被忽略的容器 `git add -A` 进去。
...
- **会话与工作目录** —— 切换项目、恢复或删除会话、搜索历史会话，还可以直接从侧栏让一个会话拥有自己的 [git 工作树](#会话级-worktree)。
- **Git 集成** —— 当前分支感知、每轮**检查点时间线**、diff 查看器、跨分支的撤销/检出。在工作树内，输入框显示的是树名，而不是一个可以切换的分支。
- **破坏性操作要说清影响范围** —— 删除工作树会话会问是否连树与分支一并移除（有别的会话还占用该树时直接拒绝），删除项目会对其中的工作树问同样的问题；所有确认框（移除工作树、删除分组、检查点恢复、删除记忆事实）都是同一个对话框，把路径、分支和未提交文件列表作为事实呈现——「没有未提交文件」与「读不到 git 状态」不会被混为一谈。
