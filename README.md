...
- **One tree per task (git worktrees)** — a session can run in its own linked git worktree on its own branch, created from the desktop sidebar or with `cluxmate worktree …`, so parallel sessions stop fighting over the same files. Inside a tree the composer stops offering branch switches (the tree *is* its branch) and the project's branch list hides what other trees hold; deleting the session can take the tree with it. See [Session worktrees](#session-worktrees).
...
Config under `<cwd>/.cluxmate/` belongs to *the repository*, not to *you*: hooks run shell commands, `mcp.json` spawns subprocesses, `lsp.json` can install toolchains, `skills/` and `agents/` reach the model's context, and `permissions.json` can pre-authorize tools. So the first time the **desktop or the TUI** opens a directory that **actually ships such config**, it asks once, with three answers: trust and remember / trust this run only / do not trust. A headless run cannot ask — `cluxmate -p "..."`, and `cluxmate repl` without a TTY, print one warning line on stderr and run with that config withheld. **Upgrade effect:** an existing `<cwd>/.cluxmate/` therefore stops being loaded until you trust the directory — grant it with `cluxmate trust add` (the scripted / CI path, e.g. `cd "$REPO" && cluxmate trust add .`) or answer the prompt once in the desktop or the TUI.

While the directory is untrusted — "do not trust" and "not answered yet" behave the same — none of that project-level config is read, and `permissions.json` is not written either: a rule that could not be persisted is a rule nothing would ever read back. So the approval card does not offer a bare "always allow": for a call that *could* be remembered it offers **"always allow & trust this project"** (one click that trusts the directory and stores the rule), and for a directory you explicitly denied it offers only the deliberate **"Trust this directory"** step — a recorded denial is never undone by the same click that grants a tool. Global `~/.cluxmate/` config and the built-in defaults are unaffected. Untrusted is **not** read-only: file writes, the sandbox boundaries and the approval tiers behave exactly as before. The decision is recorded in `~/.cluxmate/trust.json` (per directory; not a writable location, so the model cannot change it).
...
- **Desktop** — a card on first contact, a clickable **"not trusted"** badge in the working-directory bar (the one signal a directory that ships no config ever gets), and Settings → Project trust to review the decisions and revoke them.
...
The tree lands at `<repo>/.worktrees/<slug>` on a new branch `cluxmate/<slug>` (slug from `--name`, else `--title`, else `wt`; a taken name becomes `-2`, `-3`…), and `.worktrees/` goes into `<git-common-dir>/info/exclude` so the main tree never sees it as untracked — that ignore is a **precondition, not a side effect**: `create` writes it *before* the tree exists and fails with `exclude-failed` if git still would not ignore the container, because an unignored container is `git add -A` recording every checkout as an embedded repository. `create` refuses a main tree with uncommitted changes (`--allow-dirty` overrides); `remove` refuses a dirty worktree (`--force` discards it) and keeps the branch with `--keep-branch`. Inside the tree, `<worktree>/.cluxmate/` holds runtime state only — trust, permissions, skills, MCP, hooks and `AGENTS.md` stay the **main repository's** files.

### Driving them from the desktop

- **Create** — "New session in a git worktree" on a project (its header, its menu, or the group's) opens one dialog: a name (the directory and branch are derived from it), a base branch, and an optional branch name. A main tree with uncommitted changes blocks creation, and the dialog offers to commit them as `chore: WIP` or stash them right there. The entry disappears only for a project *confirmed* not to be a repository.
- **See where you are** — the session carries a badge with the tree's name (its branch in the tooltip) and lives in the project's own group.
- **Remove** — the session's context menu — and deleting the session itself — asks whether to take the tree with it (tree directory + branch + session, or session alone, with a note that the tree is then only reachable via `cluxmate worktree remove`). Deleting a whole project asks the same question for every tree inside it, and reports any tree it could not remove instead of claiming success.
- **Safety rails** — a removal is refused while *another* session still lives in that tree; the composer's branch pill is **disabled inside a worktree** and names the tree (a tree is pinned to the branch it was created on), while the project tree's dropdown hides the branches linked worktrees currently hold; and a new *plain* session never inherits a worktree directory — it starts in the project.
- **The container is CluxMate's, not your work** — the desktop never counts `.worktrees/` as one of your uncommitted changes, and the "commit the changes" path refuses to `git add -A` an unignored container.
...
- **Sessions & working directories** — switch projects, resume or delete sessions, search past sessions, and give a session its own [git worktree](#session-worktrees) from the sidebar.
- **Git integration** — current branch awareness, per-turn **checkpoint timeline**, diff viewer, and undo/checkout across branches. Inside a worktree the composer names the tree instead of offering a branch switch.
- **Destructive actions state their blast radius** — deleting a session that lives in a worktree asks whether the tree and its branch go with it (and refuses while another session still uses it), deleting a project asks the same for the trees inside it, and every confirmation — worktree removal, group delete, checkpoint restore, memory-fact delete — is one dialog that carries the path, branch and uncommitted-file list as facts — with "no uncommitted files" and "could not read git" kept visibly distinct.
